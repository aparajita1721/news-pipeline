...
DB_URL = os.getenv(
    "DATABASE_URL",
    "postgresql://airflow:airflow@postgres:5432/newsdb"
)
...
def task_extract(**context) -> None:
    ...
    context["ti"].xcom_push(key="raw_articles", value=[a.model_dump() for a in raw_articles])
...
def task_transform(**context) -> None:
    ...
    context["ti"].xcom_push(key="clean_articles", value=[a.model_dump() for a in clean_articles])
